...
entry points are `forward`, `inverse`, `lowring_log_kr`, the coordinate
functions in `src/fftloggin/grids.py`, and the unequal-time (double spherical
Bessel) transforms `double_spherical_bessel_table` and `unequal_time_kernel`
in `src/fftloggin/cosmology.py`. Kernel instances are frozen JAX pytrees with
scalar data leaves.
